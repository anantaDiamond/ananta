...
public class XInterceptor extends HandlerInterceptorAdapter {
	...
	private static final Logger log = LoggerFactory.getLogger(XInterceptor.class);
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		log.info("XInterceptor.preHandle URL: " + request.getRequestURL() + ":: Read session token key :" + request.getSession().getAttribute(TOKEN_KEY));

		if (null != request.getSession()) {
			log.info("XInterceptor.preHandle URL: " + request.getRequestURL() + ":: Method :" + request.getMethod()+":: Read request token key :" + request.getParameter(TOKEN_KEY));
				...
				if (null != request.getMethod() && request.getMethod().equals("POST")) {
					...
					if (null != token && !StringUtils.isEmpty(token)) {
						if (request.getSession().getAttribute(TOKEN_KEY).equals(token)) {
							...
							log.info("XInterceptor.preHandle URL: " + request.getRequestURL() + ":: Generated token key : " + tokenKey);
						} else {				
		...
		if (handler instanceof HandlerMethod) {
			...
			if (hm.getBean() instanceof Secure) {
				...
				if (session == null) {
					response.sendRedirect(request.getContextPath() + SignInCtl.PATH);
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler, ModelAndView modelAndView) throws Exception {
		 log.info("XInterceptor.postHandle URL: " + request.getRequestURL());
	}

	@Override
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) throws Exception {
		log.info("XInterceptor.afterCompletion URL: " + request.getRequestURL() + ":: elapsed time= " + (System.currentTimeMillis() - (Long) request.getAttribute(TIME_KEY)));
	}
