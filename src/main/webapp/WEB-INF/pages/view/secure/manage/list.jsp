...
<script type="text/javascript">
	function filters(status) {
		location.href = "${pageContext.request.contextPath}/history?status="+ status;
	}
</script>
<f:form action="">
						...
						<tbody>
							<c:forEach var="cus" items="${cusOrder}" varStatus="i">
								<tr style="height: 50px;">
									<td>${cus.value[0].createDate}</td>
									<td><x:a href="/manage/detail?orderId=${cus.key}">${cus.value[0].customerName}</x:a></td>
									<td>${c_orderStatusDescription[cus.value[0].status]}</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
						...
						<nav>
							<ul class="pagination justify-content-center">
								...
								<c:forEach begin="1" end="${page}" varStatus="i">
									<li class="page-item"><a class="page-link" href="<%=request.getContextPath() %>/history?page=${i.index}">${i.index}</a></li>
								</c:forEach>
								<li class="page-item"><a class="page-link" href="#"
